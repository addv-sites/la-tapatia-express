...
## Carrito como pantalla propia + pestañas de categoría con scroll-spy (2026-09-26)

- [x] `menu/`: el carrito (`#pedido`) era literalmente el final del scroll del menú — se sentía como "llegué al fondo de la página", no como cambiar de vista (a diferencia de Perfil, que sí es una pantalla propia). Ahora es un overlay `position:fixed` a pantalla completa que entra desde abajo (500ms, misma curva suave que el visor de imagen), con su propio header "← Menú" + "Tu pedido". Se abre desde: el tab "Pedido" del menú inferior, un botón flotante nuevo (`#fab-cart`, visible solo si hay ≥1 producto en el carrito y el overlay no está ya abierto), o llegando con `menu/#pedido` desde Home/Ubicación (sin pushState extra en ese caso, para que un solo botón atrás regrese a la página de origen). Botón atrás del celular cierra el overlay (mismo patrón `history.pushState`/`popstate` que el visor de imagen).
- [x] Las 6 pastillas de categoría no reflejaban nunca en qué sección estabas, ni al tocarlas ni al hacer scroll. Se agregó scroll-spy con `IntersectionObserver`: la pastilla de la sección visible se resalta en rojo sólido y se autocentra en el carril horizontal; se sacó "Mi pedido" del carril (ya no es una categoría de contenido, ahora vive en el tab inferior + botón flotante).
- Crítica aplicada con criterio Impeccable (modo Operate — completar tarea > decoración, estado del sistema siempre visible).
- Propuesta de referencia (interactiva): https://claude.ai/code/artifact/862ff861-38c9-449e-afda-15bfc67953ae
- Validado local: `/menu/` y `/menu/#pedido` responden 200, `menu-page.js` pasa `node --check`, clases nuevas confirmadas en `css/site.css` compilado.
